...
let emits = defineEmits(['like_tracker'])
...
watch(() => props.product.likes , () => load_likes(props.product.likes))

</script>

<template>
    <div class="w-full h-auto center flex-col gap-2 bg-white pt-5 border  border-gray-100 relative transition-custom hover:border-theme hover:shadow-theme-150 hover:z-1 ">
        <div class="w-full h-[200px] max-[470px]:h-[145px] center">
            ...
            <div class="absolute top-4 left-4 center flex-col gap-2">
                <div class="size-[7px] rounded-full shadow-sm" v-for="(color, index) in product.colors" :key="index"
                    :style="{ backgroundColor: color }"></div>
            </div>
            <nuxt-link :to="{ path: `/products/${product.id}`, query: { type: product.type_id } }"
                class="w-full h-full">
                <NuxtImg lazy="true" :src="useRuntimeConfig().public.BASEURL + product.image" class="w-full h-full object-contain"
                    alt="" />
            </nuxt-link>
        </div>
        <nuxt-link :to="{ path: `/products/${product.id}`, query: { type: product.type_id } }"
            class="w-full gap-2 px-4 py-3 pb-2">
            <p class="w-full text-title font-medium text-[15px]/6 h-[50px] overflow-hidden text-ellipsis line-clamp-2">
                {{ product.title }}</p>
            <div class="w-full flex items-center justify-between py-2">
                <p class="text-[14px] text-gray-400 font-medium" :class="{ 'text-theme': product.count == 0 }">{{
                    product.count != 0 ? `${product.count} عدد موجود` : 'ناموجود' }}</p>
                <p class="text-[12px] font-medium center text-[#3d82f4] bg-[#3d83f415] rounded-full py-[2px] px-2">
                    برند {{ product.brand }}
                </p>
            </div>
            <div class="w-full flex items-center justify-end pt-3">
