...
// Ref to store the formatted price values
const formattedMinPrice = ref(formatWithCommas(price.value[0]));
const formattedMaxPrice = ref(formatWithCommas(price.value[1]));

// Watch for changes in the filter values and update the formatted inputs
watch(() => price.value[0], (newMin) => {
    formattedMinPrice.value = formatWithCommas(newMin);
});

watch(() => price.value[1], (newMax) => {
    formattedMaxPrice.value = formatWithCommas(newMax);
});

// Handle input changes (only allow numbers and update raw values)
const formatPrice = (type, event) => {
    // Remove anything that isn't a digit
    const rawValue = event.target.value.replace(/[^\d]/g, '');

    if (type === 'min') {
        price.value[0] = parseInt(rawValue, 10) || 0; // Update the min price
    } else if (type === 'max') {
        price.value[1] = parseInt(rawValue, 10) || 0; // Update the max price
    }

    // Update the formatted input values
    if (type === 'min') {
        formattedMinPrice.value = formatWithCommas(price.value[0]);
    } else if (type === 'max') {
        formattedMaxPrice.value = formatWithCommas(price.value[1]);
    }
};
</script>
...
<template>
    <div class="w-full flex items-center justify-between gap-2">
        <span class="text-gray-400 text-[15px] font-medium">از</span>
        <input type="text" v-model="formattedMinPrice" @input="formatPrice('min', $event)"
            class="w-5/7 border-b border-gray-200 text-left h-[45px] text-[22px] px-1 font-heavy text-title">
    ...
    <div class="w-full flex items-center justify-between gap-2">
        <span class="text-gray-400 text-[15px] font-medium">تا</span>
        <input type="text" v-model="formattedMaxPrice" @input="formatPrice('max', $event)"
            class="w-5/7 border-b border-gray-200 text-left h-[45px] text-[22px] px-1 font-heavy text-title">
        <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 2" viewBox="0 0 51.29 27.19" width="51" height="27"
            class="fill-gray-400 w-5 h-5">
            <path
    ...
    <div class="w-full px-1 py-2" :style="{ direction: 'ltr' }">
        <ClientOnly>
            <Slider v-model="price" :min="0" :max="max_price" :tooltips="false" />
        </ClientOnly>
    </div>
</template>
