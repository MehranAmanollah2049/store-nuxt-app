...
<template>
    <div class="w-1/4 flex items-center justify-center gap-3 max-[830px]:w-2/4 max-[830px]:justify-start max-[460px]:flex-col max-[460px]:justify-center max-[350px]:w-full max-[360px]:flex-row max-[360px]:justify-start">
        <div class="size-[48px] center rounded-full" :class="{
            'bg-[#f1f3ff]': theme == 'default',
            'bg-[#00c07318]': theme == 'success',
            'bg-[#ed19431a]': theme == 'error',
            'bg-[#ff990021]': theme == 'orange',
        }">
            <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width="512"
                height="512" class="size-[25px] transform-[translateY(-1px)]" :class="{
                    'fill-[#4a6dff]': theme == 'default',
                    'fill-[#00c073]': theme == 'success',
                    'fill-[#ed1943]': theme == 'error',
                    'fill-[#ff9900]': theme == 'orange',
                }">
                <path
        ...
        <div class="flex items-start justify-center flex-col max-[460px]:items-center max-[360px]:items-start">
            <p class="text-gray-400 text-[14px] font-medium -mb-1">{{ title }}</p>
            <span class="text-title font-heavy text-[19px]" v-if="!loading">{{ count }}</span>
            <span class="w-[45px] h-[13px] rounded-full bg-gray-200 animation-pluse mt-[10px]" v-else></span>
        </div>
    </div>
</template>