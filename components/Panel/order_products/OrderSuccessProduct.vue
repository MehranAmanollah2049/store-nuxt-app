...
<template>
    <div
        ...
        <nuxt-link :to="`/products/${product.product_id}?type=${product.type_id}`"
            ...
            <NuxtImg lazy="true" :src="useRuntimeConfig().public.BASEURL + product.image"
            ...
            <div class="w-[calc(100%-160px)] flex items-start justify-center flex-col gap-3 max-[480px]:w-full">
                ...
                <div class="h-[25px] text-[13px] center px-2 pt-[1px] rounded-full cursor-pointer font-medium bg-[#e9edff] text-[#4a6dff]">
                     {{ product.count }} عدد
                </div>
