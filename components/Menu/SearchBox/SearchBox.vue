...
<template>
    <div class="w-[calc(100%-83px)] h-[40px] relative z-100 peer group" :class="{ 'active': isActive }">
        <div  class="w-full max-[800px]:group-[.active]:w-[calc(100%+80px)] max-[800px]:group-[.active]:-right-[78px] absolute right-0 bg-white rounded-lg min-h-full overflow-x-hidden overflow-y-auto max-h-[calc(100vh-80px)] transition-custom group-[.active]:px-3">
            <div @click="isActive = true" class="w-full h-[40px] relative rounded-md transition-custom border-b border-transparent px-3 pr-9 center bg-gray-200/50 group-[.active]:rounded-none group-[.active]:h-[51px] group-[.active]:bg-white group-[.active]:border-gray-200">
            ...
            <Transition>
                <div class="w-full py-4 px-3" v-if="isActive">
                    ...
                    <div class="w-full pt-5 center flex-col gap-5">
                        ...
                        <SearchResult v-else v-for="product in products" @removeSearch="remove_search" :key="product.id" :search="search"
                            :product="product" />
                    </div>
                </div>
            </Transition>
